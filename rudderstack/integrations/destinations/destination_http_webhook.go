...
func init() {
	...
	schema := map[string]*schema.Schema{
		"webhook_url": {
			...
			Required:         true,
			Description:      "Enter the HTTP endpoint where RudderStack will send the events.",
			ValidateDiagFunc: c.StringMatchesRegexp("(^\\{\\{.*\\|\\|(.*)\\}\\}$)|(^env[.].+)|^(?:http(s)?:\\/\\/)?[\\w.-]+(?:\\.[\\w\\.-]+)+[\\w\\-\\._~:/?#[\\]@!\\$&'\\(\\)\\*\\+,;=.]+$"),
		},
		"webhook_method": {
			Type:             schema.TypeString,
			Optional:         true,
			Description:      "The HTTP method of the request sent to the configured endpoint. By default, POST is used.",
		...
		"headers": {
			...
			Optional:    true,
			Sensitive:   true,
			Description: "Add custom headers for your events. These headers will be added to the request made from RudderStack to your HTTP webhook endpoint.",
	...
	c.Destinations.Register("http_webhook", c.ConfigMeta{
		APIType:      "HTTP_WEBHOOK",
