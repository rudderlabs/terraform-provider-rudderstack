...
.PHONY: test-ci
test-ci: test-ci-preflight
	go test ./... -timeout 30m

.PHONY: test-ci-preflight
test-ci-preflight:
	@if ! command -v terraform >/dev/null 2>&1; then \
		echo "make test-ci requires the Terraform CLI on PATH." >&2; \
		echo "Several non-acceptance tests use terraform-plugin-sdk resource.UnitTest, which invokes the Terraform CLI." >&2; \
		echo "Install Terraform in the DevBox image or local environment before running make test-ci." >&2; \
		exit 1; \
	fi
	@terraform version
