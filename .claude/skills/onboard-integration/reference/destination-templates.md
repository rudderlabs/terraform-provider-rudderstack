...
- **Do NOT extract the `connection_mode` block into helper functions** (e.g. `connectionModeSchema()` / `connectionModeEntry()`). Even though the 10+ near-identical source-type entries look like obvious DRY candidates, inline each one — that's the convention every existing destination follows (see `destination_attentive_tag.go`, `destination_braze.go`, `destination_linkedin_ads.go`). Reasons: (1) per-destination divergence is common (different regex like `^(cloud)$` vs `(^env[.].*)|^(cloud|device|hybrid)$`, different source-type sets, occasional per-field descriptions) and a shared helper falsely implies uniformity; (2) helpers land in the package-global `destinations` namespace with generic-sounding names that look reusable but are tied to one destination's rules, which misleads the next reader.
