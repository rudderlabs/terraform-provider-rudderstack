...
- Map Customer.io `apiVersion` with plain `c.Simple("apiVersion", "api_version")`; do not add a custom property helper to skip default `v1` writes or inject `v1` on reads unless explicitly re-requested.
