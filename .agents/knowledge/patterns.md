...
- Destination GET-response assertions should be derived from the destination `ConfigMeta` and ignore only response fields that map to sensitive Terraform schema values; broad `compareConfig` loosening hides legitimate missing allowlisted fields.
