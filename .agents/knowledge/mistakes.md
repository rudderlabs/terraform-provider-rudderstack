...
- Preserving schema-sensitive values is not enough if destination credential fields are missing `Sensitive: true`; observed E2E failures included API-redacted `eventKey`, `apiKey`, `caCertificate`, AWS `accessKeyID`, and Kinesis `key_based_authentication` metadata. Mark API-redacted credential-like schema fields sensitive and include nested block container metadata in import ignores when a block contains sensitive children.
