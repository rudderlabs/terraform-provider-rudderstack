...
- `configs.TestConfig.APIResponseOptionalFields` only relaxes the explicit live API comparison; it does not bypass the Terraform SDK acceptance harness's automatic post-apply empty-plan check. Live fixtures must round-trip through API reads: omit unsupported explicit options, leave immutable update values unchanged, and configure backend-omitted boolean defaults to their read-back zero value when outbound serialization still needs coverage.
