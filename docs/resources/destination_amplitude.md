...
- `sdk_version` (Block List, Max: 1) Choose the Amplitude Browser SDK version to load for web (JavaScript) sources. Version `1` is used when this block is omitted. (see [below for nested schema](#nestedblock--config--sdk_version))
