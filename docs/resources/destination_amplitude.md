...
- `sdk_version` (Block List, Max: 1) Choose the Amplitude Browser SDK version to load for web (JavaScript) sources. Defaults to `2` when this block is omitted. (see [below for nested schema](#nestedblock--config--sdk_version))
...
- `web` (Number) Amplitude Browser SDK version for web sources. Valid values are `1` and `2`. Defaults to `2`.
