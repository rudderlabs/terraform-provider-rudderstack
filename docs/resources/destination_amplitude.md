...
`sdk_version.web` defaults to `1` to preserve existing Amplitude destination behavior unless you explicitly opt into `2`.
...
- `sdk_version` (Block List, Max: 1) Choose the Amplitude Browser SDK version to load for web (JavaScript) sources. Defaults to `1` when omitted. (see [below for nested schema](#nestedblock--config--sdk_version))
